...
const OfferCard = ({ product }) => {
  ...
  const calculateSavings = () => {
    if (selectedSize?.lisedPrice && selectedSize?.price) {
      return (selectedSize?.lisedPrice - selectedSize?.price).toFixed(2);
    }
    return null;
  };
  ...
  return (
    <div className="pro-card shadow rounded h-100 mb-3">
      {percentageOff && (
        <div className="product-discount">{percentageOff.toFixed(2)}% OFF</div>
      )}
      ...
      <div className="product-info">
        <h3 className="product-title" onClick={handleImageClick} style={{textAlign:"center"}}>
          {product.productName}
        </h3>
        ...
        {savings && (
          <div className="product-savings" style={{textAlign:"center"}}> 
 ...
 {/* <button
      ...
      >
        {loadingButton === product._id ? (
          ...
          <>
            <FaShoppingCart className="me-2 fasshopping "/> Add to Cart
          </>
        )}
      </button> */}
